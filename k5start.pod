...
B<k5start> B<-P> B<-f> I<keytab> [B<-nqstv>]
[B<-f> I<keytab>]
[B<-H> I<minutes>]
[B<-I> I<service instance>]
[B<-K> I<minutes>]
...
regular command-line argument or via the B<-P> option; the B<-u> and B<-i>
...
When B<-P> is given, B<k5start> will not expect a principal name to be
...
Do the same thing, but determine the principal from the keytab:
    ...
    k5start -f /etc/krb5.keytab -P -- sh -c 'ls -l $KRB5CCNAME'
