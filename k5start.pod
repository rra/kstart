...
user running B<kstart> in the default local realm.
...
Ignored, present for option compatibility with B<kstart>.
...
This uses the F</etc/krb5.keytab> keytab to obtain a ticket granting
ticket for the principal host/example.stanford.edu.  The lifetime is 10
hours and the program wakes up every 10 minutes to check if the ticket is
about to expire.
