...
B<k5start> [B<-bhnqstv>] [B<-f> I<keytab>] [B<-H> I<minutes>]
...
B<k5start> B<-U> B<-f> I<keytab> [B<-bhnqstv>] [B<-H> I<minutes>]
    [B<-I> I<service instance>] [B<-K> I<minutes>] [B<-k> I<ticket file>]
    [B<-l> I<time string>] [B<-p> I<pid file>] [B<-r> I<service realm>]
...
at least I<minutes> minutes.  If such a ticket is found, exit immediately
with status 0.  Otherwise, try to obtain a new ticket.  Cannot be used
with B<-K>.
