...
=head1 EXAMPLE
    ...
    kstart -k /tmp/service.tgt -f /etc/srvtab -K 10 -l 600 \
        rcmd.example

This uses the F</etc/srvtab> srvtab to obtain a ticket granting ticket for
the principal rcmd.example.  The lifetime is 600 minutes and the program
wakes up every 10 minutes to check if the ticket is about to expire.
