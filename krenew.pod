...
=for stopwords
-bhitv aklog AFS OpenSSH PAG HUP ALRM KRB5CCNAME AKLOG kstart afslog
Allbery Bense

=head1 SYNOPSIS

B<krenew> [B<-bhitv>] [B<-c> I<child pid file>] [B<-H> I<minutes>]
...
again after the next wakeup interval (see B<-K>).  This is useful if some
...
before or less than two minutes before the next wakeup, and renews the
ticket if needed.  If this option is not given but a command was given on
the command line, the default interval is 60 minutes (1 hour).
