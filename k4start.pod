...
=for stopwords
-bhnqstv srvtab username kinit LDAP AFS aklog HUP KRBTKFILE AKLOG TGT PAG
init afslog kstart krenew Bense Allbery Navid Golpayegani

=head1 SYNOPSIS

B<k4start> [B<-bhnqstv>] [B<-c> I<child pid file>] [B<-f> I<srvtab>]
    [B<-g> I<group>] [B<-H> I<minutes>] [B<-I> I<service instance>]
    [B<-i> I<client instance>] [B<-K> I<minutes>] [B<-k> I<ticket file>]
    [B<-l> I<minutes>] [B<-m> I<mode>] [B<-o> I<owner>]
...
B<kinit>, but it is primarily intended to be used by programs that want to
...
When running a command, B<k4start> propagates HUP, TERM, and QUIT signals
to the child process and does not exit when those signals are received.
(If the propagated signal causes the child process to exit, B<k4start>
will then exit.)  This allows B<k4start> to react properly when run under
...
If this flag is given, B<k4start> will also change directories to C</>.
...
If this option is used with a command or with the B<-K> option, there may
be a short window after each authentication during which the group
ownership of the cache will be the default group of the B<k4start>
process.
...
before or less than two minutes after the next wakeup, and gets a new
ticket if needed.
...
permissions of the ticket cache will be C<600>.  Setting a I<mode> that
does not allow B<k4start> to read or write to the ticket cache will cause
B<k4start> to fail and exit when using the B<-K> option or running a
command.

=item B<-n>

Regardless of the setting of the environment variables AKLOG and
KINIT_PROG, don't run the program.
...
the cache will be the user the B<k4start> process is running as.
...
risk.  You normally want to use a srvtab and the B<-f> option instead.
...
If B<k4start> has been built with AFS setpag() support and a command was
given on the command line, B<k4start> will create a new PAG before
...
principal C<rcmd.example>, putting the ticket cache in
F</tmp/service.tgt>.  The lifetime is 600 minutes and the program wakes up
...
This uses F</var/run/k4start.pid> as the PID file and obtains
C<rcmd.example> tickets from the system srvtab file.  B<k4start> would
...
If the environment variable AKLOG is set, its value will be taken as the
path to a program and that program will be automatically executed after
the ticket-granting ticket has been retrieved (unless B<-n> was given).
...
line, B<k4start> will use the environment variable KRBTKFILE to determine
...
k5start(1), kinit(1), krenew(1)

The kstart web page at L<http://www.eyrie.org/~eagle/software/kstart/>
will have the current version of B<k4start>, B<k5start>, and B<krenew>.
